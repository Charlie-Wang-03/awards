...
/-!
# Closing the mixed four-centre interior branch

The geometric work now supplies the weakened hidden-angle estimate

  ((n-1)-delta)*lambda <= angle(s,b,c).

The short interior arithmetic theorem was strengthened to accept exactly this
one-remainder loss.  This file combines the two pieces and closes the complete
interior mixed-support branch.
-/
...
theorem no_mixed_fourth_inside_of_weakened_hidden_angle_fin_four
    {p : Fin 4 → Plane}
...
theorem no_mixed_fourth_inside_fin_four
    {p : Fin 4 → Plane}
    (hp : Function.Injective p)
    (hcap : AngleCap p lam)
    {lam t delta : ℝ} {n : ℕ}
    ...
    False := by
  have hhidden :=
    mixed_inside_hidden_angle_weakened_fin_four
      hp hcap hn hdelta0 hdeltaHalf ht hlam
      hsa hsb hsc hab hac hbc
      Cs Ca Cb hS hA hB hsupA hsupB hinside
  exact no_mixed_fourth_inside_of_weakened_hidden_angle_fin_four
    hp hcap hn hdelta0 hdeltaHalf ht hlam
    hsa hsb hsc hab hac hbc hinside hhidden

#print axioms no_mixed_fourth_inside_of_weakened_hidden_angle_fin_four
#print axioms no_mixed_fourth_inside_fin_four

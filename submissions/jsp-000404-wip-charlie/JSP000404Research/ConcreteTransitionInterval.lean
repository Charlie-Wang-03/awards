...
/-!
# Quantitative transition interval for every high-exponent centre

ConcreteHighExponentExposure proves that every centre with exponent at least
n-2 is strictly exposed.  The proof actually contains more information.

There is a unique sign-transition quotient qe and corresponding normalized
projective gap ge such that

  qe != 0,
  qe <= t*ge,
  ge > 0,

and, after lifting one sign block by pi if necessary, every displacement ray
from the centre has one common sign on an interval of width

  pi*(1-ge).

Therefore the dual strict-support arc has angular length exactly pi*ge.

Unlike ConcreteOneSupportInterval, no support=1 hypothesis is imposed and no
identity qe=k+1 is claimed.  This is the quantitative certificate needed to
pack the actual transition quotients of all top-layer centres.
-/
...
structure HighExponentTransitionIntervalCertificate
...
/-- Every concrete centre in the top three dyadic layers has a quantitative
transition-gap interval certificate. -/
theorem exists_highExponentTransitionIntervalCertificate
    {V : Type*} [LinearOrder V] [Fintype V]
    {p : V → Plane}
    ...
    Nonempty
      (HighExponentTransitionIntervalCertificate hp t i C) := by
...
#print axioms exists_highExponentTransitionIntervalCertificate

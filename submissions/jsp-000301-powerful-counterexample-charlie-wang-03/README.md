...
The upstream catalog records the counterexample `12167 = 23^3` and `12168 = 2^3 * 3^2 * 13^2`. This package does **not** claim discovery of that historical counterexample. It supplies a Lean 4 formalization, statement-alignment audit, natural-language exposition, and reproducibility evidence.
...
A successful Lean kernel check establishes that the encoded theorem follows in the pinned formal environment. It is not, by itself, independent verification of source alignment, attribution, or prize eligibility. Those remain subject to upstream review.
